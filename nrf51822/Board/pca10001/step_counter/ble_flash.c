...
//seattle-test+>>>
/*
uint32_t W1_flash_page_validate(uint8_t page_num)
{
    uint32_t * p_page;
    uint32_t * p_curr_addr;
    uint32_t   flash_header;
    ...
    {		
		...
		// Read number of elements
		//p_curr_addr++;		
 //   *p_word_offset = (uint8_t)(*(p_curr_addr));

		return NRF_SUCCESS;

}
*/
uint32_t W1_flash_words_write(uint8_t page_num, uint8_t word_offset, uint32_t * p_in_array, uint8_t word_count)
{
    int        i;
    uint32_t * p_page;
    uint32_t * p_curr_addr;
    ...
    //uint32_t   flash_header;
		...
		p_page      = (uint32_t *)(BLE_FLASH_PAGE_SIZE * page_num);    
    p_curr_addr = p_page + word_offset + 2;
...
uint32_t W1_flash_words_read(uint8_t page_num, uint32_t * p_out_array, uint8_t word_offset)
{
		int        byte_count;
    uint32_t * p_page;
    uint32_t * p_curr_addr;
...
//    uint16_t   calc_crc;
    ...
    // Read data
    byte_count = word_offset * sizeof(uint32_t);
    memcpy(p_out_array, p_curr_addr, byte_count);
...
uint32_t W1_flash_words_read2(uint8_t page_num, uint32_t * p_out_array, uint8_t word_offset, uint8_t word_count)
{
		int        byte_count;
    uint32_t * p_page;
    uint32_t * p_curr_addr;
    ...
    if (tmp != BLE_FLASH_MAGIC_NUMBER)
    {
		...
		//p_curr_addr++;
		//p_curr_addr++;
		p_curr_addr = p_curr_addr + word_offset + 2;
    // Read data
